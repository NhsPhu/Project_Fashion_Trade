...
const InventoryService = {
    ...
    getStock: async (variantId, warehouseId) => {
        const response = await apiClient.get(`${API_PREFIX}/${variantId}/${warehouseId}`);
        return response.data;
    },

    getAllInventory: async () => {
        const response = await apiClient.get(`${API_PREFIX}/all`);
        return response.data;
    },
    ...
    updateStock: async ({ variantId, warehouseId, quantity, action }) => {
        ...
        const response = await apiClient.post(API_PREFIX, payload);
        return response.data;
    },
};
